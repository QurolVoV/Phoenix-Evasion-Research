...
# Copy all files from builder and then clean up what we don't need
COPY --from=builder /build/ /app/build-temp/

# Move documentation files to app root and create defaults
RUN mkdir -p /app && \
    cp /app/build-temp/README.md /app/README.md 2>/dev/null || echo "# Phoenix-Evasion-Research" > /app/README.md && \
    cp /app/build-temp/SECURITY.md /app/SECURITY.md 2>/dev/null || echo "# Security Policy" > /app/SECURITY.md && \
    cp /app/build-temp/LICENSE /app/LICENSE 2>/dev/null || echo "MIT License" > /app/LICENSE && \
    rm -rf /app/build-temp/
